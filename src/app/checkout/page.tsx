...
function Progress({ currentStep }: { currentStep: number }) {
  ...
  return <ol className="mx-auto flex w-full max-w-2xl items-center" aria-label="Checkout progress">{steps.map((step, index) => <li className="flex flex-1 items-center last:flex-none" key={step}><div className="flex flex-col items-center"><span className={`grid h-8 w-8 place-items-center rounded-full text-xs font-bold transition-all duration-300 ${index < currentStep ? "bg-brand text-white shadow-[0_4px_10px_rgba(56,43,213,.3)]" : index === currentStep ? "bg-brand text-white ring-4 ring-brand-light" : "bg-slate-200 text-slate-500"}`}>{index < currentStep ? <Icon name="check" className="h-4 w-4 fill-none stroke-current stroke-[2.5]" /> : index + 1}</span><span className={`mt-2 text-xs font-semibold ${index === currentStep ? "text-brand" : "text-slate-500"}`}>{step}</span></div>{index < steps.length - 1 && <span className={`mx-3 mb-5 h-0.5 flex-1 transition-colors duration-300 ${index < currentStep ? "bg-brand" : "bg-slate-200"}`} />}</li>)}</ol>;
}
