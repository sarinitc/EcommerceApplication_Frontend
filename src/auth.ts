...
export const {
  ...
  providers: [
    ...
    Credentials({
      ...
      credentials: {
        account: { label: "Account type", type: "text" },
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          if (!currentUser || currentUser.accountStatus?.toUpperCase() === "BLOCKED") return null;
          if (credentials.account === "admin" && !currentUser?.roles?.includes("ADMIN")) {
            return null;
          }
  ...
  callbacks: {
    ...
    async jwt({ token, user, trigger, session }) {
      ...
      // Revalidate existing sessions against the backend, including API requests.
      if (!user) {
        if (typeof token.backendAccessToken !== "string") return null;
        try {
          const apiUrl = (process.env.API_URL ?? process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8081/api/v1").replace(/\/$/, "");
          const response = await fetch(`${apiUrl}/auth/me`, {
            headers: { Authorization: `Bearer ${token.backendAccessToken.replace(/^Bearer\s+/i, "")}` },
            cache: "no-store",
          });
